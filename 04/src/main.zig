...
const Board = struct {
    ...
    pub fn initFromBuffer(board_id: usize, buffer: []const u8) !Board {
        var board = Board{
            .id = board_id,
            .numbers = undefined,
...
pub fn main() !void {
    ...
    var board_id: usize = 0;
    while (board_input_iter.next()) |board_input| : (board_id += 1) {
        const board = try Board.initFromBuffer(board_id, board_input);
    ...
    var last_winning_number: u32 = undefined;

    var draw_iter = std.mem.split(u8, draw_inputs, ",");
    while (draw_iter.next()) |draw| {
        const number = try std.fmt.parseInt(u8, draw, 10);

        var board_index: usize = 0;
        while (board_index < boards.items.len) {
            const board: *Board = &boards.items[board_index];

            board.mark(number);

            if (board.isWinner()) {
                last_winning_board = boards.orderedRemove(board_index);
    ...
    const final_score = last_winning_board.score() * last_winning_number;

    log.info("Board {d} is the last winner, with a score of {d}!", .{ last_winning_board.id, final_score });
}
