...
pub fn main() !void {
    ...
    const steps_to_do: u8 = 100;

    var total_flash_count: usize = 0;

    for (0..steps_to_do) |step_number| {
        var flash_count_this_step: usize = 0;
        ...
        log.debug("Step: {d} Flashes: {d}", .{ step_number + 1, flash_count_this_step });

        total_flash_count += flash_count_this_step;
    }

    log.info("Total flashes after {d} steps: {d}", .{ steps_to_do, total_flash_count });
}
